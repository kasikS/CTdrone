...
{
    nrf24l_write_reg(NRF24L_STATUS,
            NRF24L_STATUS_RX_DR | NRF24L_STATUS_TX_DS | NRF24L_STATUS_MAX_RT);
}
...
static void nrf24l_set_mode(enum MODE mode)
{
    ...
    switch(mode)
    {
        ...
        case RX:
            ...
            nrf24l_clear_irq();
            nrf24l_ce_disable();
            nrf24l_write_reg(NRF24L_CONFIG,
                    DEFAULT_CONF | NRF24L_CONF_PWR_UP | NRF24L_CONF_PRIM_RX);
            nrf24l_ce_enable();
            delay_us(12);
...
int nrf24l_init(void)
{
    ...
    spi_conf.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_8; // 48MHz/8 = 6MHz
    ...
    if(!xTaskCreate(nrf24l_irq_handler_task, NULL, configMINIMAL_STACK_SIZE,
                NULL, 2, NULL))
        return pdFALSE;

    if(!xTaskCreate(nrf24l_transmitter_task, NULL, configMINIMAL_STACK_SIZE,
                NULL, 2, NULL))
        return pdFALSE;
    ...
    nrf24l_write_reg(NRF24L_EN_AA, NRF24L_EN_AA_P0);

    // Enable RX addresses
    nrf24l_write_reg(NRF24L_EN_RXADDR, NRF24L_RXADDR_P0);

    // Auto retransmit delay: 750 us and up to 5 retransmit trials
    nrf24l_write_reg(NRF24L_SETUP_RETR, NRF24L_SET_RETR_ARD(NRF24L_ARD_750_US) |
                                        NRF24L_SET_RETR_ARC(5));

    // Dynamic length configurations: No dynamic length
    nrf24l_write_reg(NRF24L_DYNPD, NRF24L_DPL_P0);
...
{
    // NOP also returns status, but it is only one byte transmitted
    return nrf24l_raw_8b(NRF24L_NOP);
}
        ...
        /*if(xSemaphoreTake(nrf24l_tx_sem, NRF24L_TICKS_WAIT) == pdTRUE) {*/
            ...
            while(xQueueReceive(nrf24l_tx_queue, (void*) &tx_buf[cnt], 0)) {
                if(++cnt == PACKET_TOTAL_SIZE) {
                    // Complete packet is ready to be transmitted
                    while(nrf24l_get_status() & NRF24L_STATUS_TX_FULL)
                        vTaskDelay(1);
                    ...
                    nrf24l_ce_enable();
                    // CE is disabled in the irq handler
                }
            }
        /*}*/
    }
