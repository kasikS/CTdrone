...
#define SERIAL_TICKS_WAIT   32


xQueueHandle serial_rx_queue;
static xQueueHandle serial_tx_queue;
static void serial_tx_task(void *parameters);
...
//with callback...??
    ...
    // FreeRTOS stuff
    serial_tx_queue = xQueueCreate(SERIAL_QUEUE_SIZE, sizeof(char));
    if(serial_tx_queue == 0)
        return pdFALSE;

    serial_rx_queue = xQueueCreate(SERIAL_QUEUE_SIZE, sizeof(char));
    if(serial_rx_queue == 0)
        return pdFALSE;

    portBASE_TYPE ret = xTaskCreate(serial_tx_task, NULL, configMINIMAL_STACK_SIZE, NULL, 2, NULL);
    if(ret != pdPASS)
        return pdFALSE;
...
{
    if(xQueueSend(serial_tx_queue, &c, SERIAL_TICKS_WAIT) != pdTRUE)
        return pdFALSE;     // probably it is errQUEUE_FULL, translate it to false
